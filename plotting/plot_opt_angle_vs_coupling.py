"""
Draw the optimal-angle / marker figure from the scan written by
base-code/03_opt_angle_vs_coupling_v2.py.

    python plotting/plot_opt_angle_vs_coupling.py
    python plotting/plot_opt_angle_vs_coupling.py -f data/opt_angle_vs_coupling_v2_....h5

With no file given, the newest opt_angle_vs_coupling_v2_*.h5 in data/ is used, so a re-run
of the scan is picked up without editing anything here. The figure itself is unchanged from
the one the scan script used to draw; only where its numbers come from has moved.
"""
...
parser = argparse.ArgumentParser(description='Plot the optimal angle vs coupling scan')
parser.add_argument('-f', '--file', type=str, default=None,
                    help='Scan .h5 (default: the newest one in data/)')
...
if args.file is None:
    candidates = sorted((repo_root / 'data').glob('opt_angle_vs_coupling_v2_*.h5'),
                        key=lambda p: p.stat().st_mtime)
    if not candidates:
        raise IOError(f'No opt_angle_vs_coupling_v2_*.h5 in {repo_root / "data"} -- '
                      f'run base-code/03_opt_angle_vs_coupling_v2.py first')
...
with h5py.File(file_path, 'r') as f:
    ...
    # Simulation
    lambR_vec                = f['Simulation/lambR_vec'][:]
    lambD_vec                = f['Simulation/lambD_vec'][:]
    theta_vec                = f['Simulation/theta_vec'][:]
    theta_opt_vec            = f['Simulation/theta_opt_vec'][:]
    theta_opt_clean_vec      = f['Simulation/theta_opt_clean_vec'][:]
    marker_opt_vec           = f['Simulation/marker_opt_vec'][:]
    marker_opt_clean_vec     = f['Simulation/marker_opt_clean_vec'][:]
    gap_Stilde_opt_vec       = f['Simulation/gap_Stilde_opt_vec'][:]
    gap_Stilde_opt_clean_vec = f['Simulation/gap_Stilde_opt_clean_vec'][:]
    gap_H_vec                = f['Simulation/gap_H_vec'][:]
    gap_theta_fixedR         = f['Simulation/gap_theta_fixedR'][:]
    id_R_fixed               = int(f['Simulation/id_R_fixed'][()])

    # Parameters, so the annotations do not have to be kept in step by hand
    M     = f['Parameters/M'][()]
    A     = f['Parameters/A'][()]
...
loger_main.info(f'lambR: {len(lambR_vec)} points, lambD: {lambD_vec.tolist()}, '
                f'theta: {len(theta_vec)} points')
